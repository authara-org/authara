...
type Challenge struct {
	...
	VerificationCodeTTL time.Duration `env:"AUTHARA_VERIFICATION_CODE_TTL,default=10m"`
...
func (c *Challenge) validate() error {
	...
	if c.VerificationCodeTTL <= 0 {
		return fmt.Errorf("AUTHARA_VERIFICATION_CODE_TTL must be > 0")
	}
	if c.VerificationCodeTTL > c.TTL {
		return fmt.Errorf(
			"AUTHARA_VERIFICATION_CODE_TTL (%s) must not exceed AUTHARA_CHALLENGE_TTL (%s)",
